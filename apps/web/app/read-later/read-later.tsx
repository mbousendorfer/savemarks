...
function splitTags(value: string) {
  return [...new Set(value.split(/[,;]/).map((tag) => tag.trim().replace(/^#+/, "").toLocaleLowerCase()).filter(Boolean))].slice(0, 20);
}

function normalizedStatus(value: string) {
  const key = value.trim().toLocaleLowerCase();
  if (["read", "done", "finished", "lu", "terminé", "termine"].includes(key)) return "read";
  if (["archived", "archive", "archivé", "archivee"].includes(key)) return "archived";
  if (["unread", "to read", "à lire", "a lire", "pending", ""].includes(key)) return key ? "unread" : undefined;
  return undefined;
}
...
function normalizeForDuplicate(value: string) {
  ...
  for (const key of [...url.searchParams.keys()]) {
    if (key.toLowerCase().startsWith("utm_") || ["s", "t", "igsh", "igshid"].includes(key.toLowerCase())) {
...
function ThemeButton() {
  ...
  return (
    <button className="rl-icon-button" onClick={toggle} aria-label={`Use ${theme === "dark" ? "light" : "dark"} theme`}>
      {theme === "dark" ? <SunIcon size={18} /> : <MoonIcon size={18} />}
    </button>
  );
}

function AddPanel({ onClose, onComplete }: { onClose: () => void; onComplete: () => void }) {
  ...
  async function addLink() {
    ...
    setBusy(false);
    if (!response.ok) return setMessage("The link could not be saved.");
    onComplete();
    onClose();
  }
  ...
  async function loadFile(file: File) {
    setMessage("");
    setErrors([]);
    if (file.size > 10 * 1024 * 1024) return setMessage("File exceeds the 10 MB limit.");
    const text = await file.text();
    setFileName(file.name);
    if (file.name.toLowerCase().endsWith(".txt")) {
      const values = text.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
      if (values.length > 25_000) return setMessage("File exceeds the 25,000 row limit.");
      setHeaders(["url"]);
    ...
    if (data.length > 25_000) return setMessage("File exceeds the 25,000 row limit.");
    const cleanHeaders = first.map((value, index) => (value || `column_${index + 1}`).replace(/^\uFEFF/, "").trim());
    const detected: Partial<Record<TargetField, number>> = {};
    for (const [target] of TARGETS) {
      const index = cleanHeaders.findIndex((header) => SYNONYMS[target].includes(header.toLocaleLowerCase().replace(/\s+/g, "_")));
      if (index >= 0) detected[target] = index;
    }
    setHeaders(cleanHeaders);
    setRows(data);
    setMapping(detected);
    if (result.errors.length) setMessage(`${result.errors.length} parsing warning(s) detected.`);
  }
  ...
  const prepared = useMemo(() => {
    ...
    rows.forEach((row, index) => {
      ...
      if (!validHttpUrl(address)) return invalid.push({ row: index + 2, error: "Invalid URL" });
      const identity = normalizeForDuplicate(address);
      if (seen.has(identity)) return invalid.push({ row: index + 2, error: "Duplicate in file" });
      seen.add(identity);
      const savedAt = value("savedAt");
      if (savedAt && Number.isNaN(new Date(savedAt).getTime())) return invalid.push({ row: index + 2, error: "Invalid ISO date" });
      const statusValue = value("status");
      const status = normalizedStatus(statusValue);
      if (statusValue && !status) return invalid.push({ row: index + 2, error: "Unknown status" });
  ...
  async function runImport() {
    if (!mapping.url && mapping.url !== 0) return setMessage("Map an URL column first.");
    setBusy(true);
    ...
    for (let offset = 0; offset < prepared.valid.length; offset += 100) {
      ...
      const response = await fetch("/api/read-later/import", {
        method: "POST",
        headers: { "content-type": "application/json" },
      ...
      if (!response.ok) {
        ...
        failures.push(...batch.map((entry) => ({ row: entry.row, error: "Batch request failed" })));
      } else {
        const payload = (await response.json()) as { results: Array<{ row: number; status: string; error?: string }> };
        created += payload.results.filter((entry) => entry.status === "created").length;
        updated += payload.results.filter((entry) => entry.status === "updated").length;
        const invalid = payload.results.filter((entry) => entry.status === "invalid");
        serverFailures += invalid.length;
        failures.push(...invalid.map((entry) => ({ row: entry.row, error: entry.error ?? "Invalid row" })));
      }
      completed += batch.length;
      setProgress(Math.round((completed / Math.max(1, prepared.valid.length)) * 100));
    }
    setErrors(failures);
    setBusy(false);
    setMessage(`Import complete · ${created} created · ${updated} updated · ${prepared.invalid.length + serverFailures} skipped`);
    onComplete();
  }

  function downloadErrors() {
    const csv = Papa.unparse(errors.map((error) => ({ row: error.row, error: error.error })));
  ...
  function downloadTemplate() {
    ...
    const anchor = document.createElement("a");
    anchor.href = URL.createObjectURL(new Blob([sample], { type: "text/csv;charset=utf-8" }));
    ...
    <div className="rl-overlay" role="dialog" aria-modal="true" aria-label="Add a Web link">
      <section className="rl-add-panel">
        ...
        <div className="rl-tabs">
          <button className={tab === "link" ? "active" : ""} onClick={() => setTab("link")}><PlusIcon /> One link</button>
          <button className={tab === "import" ? "active" : ""} onClick={() => setTab("import")}><UploadSimpleIcon /> Import</button>
        </div>
        {tab === "link" ? (
          <div className="rl-form">
            <label>URL<input autoFocus value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://example.com/article" /></label>
            <label>Tags <span>optional</span><input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="design, research" /></label>
            <button className="rl-primary" disabled={busy} onClick={() => void addLink()}>{busy ? "Saving…" : "Save link"}</button>
          </div>
        ) : (
          <div className="rl-import">
            <input ref={inputRef} type="file" accept=".csv,.txt,text/csv,text/plain" hidden onChange={(event) => event.target.files?.[0] && void loadFile(event.target.files[0])} />
            {!rows.length ? (
              <><button className="rl-dropzone" onClick={() => inputRef.current?.click()}><UploadSimpleIcon size={28} /><strong>Choose CSV or TXT</strong><span>Up to 10 MB · 25,000 rows</span></button><button className="rl-text-button" onClick={downloadTemplate}>Download SaveMarks CSV template</button></>
            ) : (
              <>
                <div className="rl-import-summary"><strong>{fileName}</strong><span>{prepared.valid.length} valid · {prepared.invalid.length} skipped</span></div>
                <div className="rl-mapping">
                  {TARGETS.map(([target, label]) => (
                    <label key={target}><span>{label}{target === "url" ? " *" : ""}</span><select value={mapping[target] ?? ""} onChange={(event) => setMapping((current) => ({ ...current, [target]: event.target.value === "" ? undefined : Number(event.target.value) }))}><option value="">Not mapped</option>{headers.map((header, index) => <option key={`${header}-${index}`} value={index}>{header}</option>)}</select></label>
                  ))}
                </div>
                <div className="rl-preview-table"><table><thead><tr>{headers.slice(0, 5).map((header) => <th key={header}>{header}</th>)}</tr></thead><tbody>{rows.slice(0, 5).map((row, index) => <tr key={index}>{row.slice(0, 5).map((value, cell) => <td key={cell}>{value}</td>)}</tr>)}</tbody></table></div>
                <button className="rl-primary" disabled={busy || !prepared.valid.length} onClick={() => void runImport()}>{busy ? `Importing ${progress}%` : `Import ${prepared.valid.length} links`}</button>
              </>
            )}
          </div>
        )}
        {message && <p className="rl-message">{message}</p>}
        {errors.length > 0 && <button className="rl-text-button" onClick={downloadErrors}>Download error report</button>}
      </section>
    </div>
  );
}
...
export function ReadLater() {
  ...
  const load = useCallback(async (append = false, pageCursor?: string | null) => {
    setLoading(true);
    const params = new URLSearchParams({ status, sort, q: query, limit: "30" });
    if (tag !== "all") params.set("tag", tag);
    if (append && pageCursor) params.set("cursor", pageCursor);
    const response = await fetch(`/api/read-later?${params}`);
    if (response.ok) {
      const payload = (await response.json()) as { items: ReadLaterItem[]; nextCursor: string | null; unreadCount: number };
  ...
  const availableTags = useMemo(
    () => [...new Set([...items.flatMap((item) => item.tags), ...(tag === "all" ? [] : [tag])])].sort(),
    [items, tag],
  );
  ...
  async function patchItem(id: string, update: { read?: boolean; archived?: boolean }) {
    const response = await fetch(`/api/bookmarks/${id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(update) });
    if (response.ok) await load(false);
  }

  async function saveTags(id: string) {
    const next = splitTags(editTags);
    const response = await fetch(`/api/bookmarks/${id}/tags`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ tags: next }) });
    if (response.ok) {
    ...
    <main className="app-shell rl-shell">
      ...
      <section className="workspace rl-workspace">
        ...
        <section className="rl-list" aria-live="polite">
          {items.map((item) => (
            <article className="rl-card" key={item.id}>
              <a className="rl-thumb" href={item.canonicalUrl} target="_blank" rel="noreferrer">{item.imageUrl ? <img src={item.imageUrl} alt="" loading="lazy" /> : <span>{(item.siteName ?? new URL(item.canonicalUrl).hostname).slice(0, 1).toUpperCase()}</span>}</a>
              <div className="rl-card-body"><div className="rl-card-meta"><span>{item.siteName ?? new URL(item.canonicalUrl).hostname}</span><i>·</i><time>{new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(item.savedAt))}</time>{item.enrichmentStatus !== "complete" && <em className={`rl-state ${item.enrichmentStatus}`}>{item.enrichmentStatus}</em>}</div><h2><a href={item.canonicalUrl} target="_blank" rel="noreferrer">{item.title ?? item.canonicalUrl}</a></h2>{item.description && <p>{item.description}</p>}<div className="rl-tags">{item.tags.map((tag) => <span key={tag}><TagIcon size={12} />{tag}</span>)}</div>{editing === item.id && <div className="rl-edit-tags"><input autoFocus value={editTags} onChange={(event) => setEditTags(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void saveTags(item.id)} /><button onClick={() => void saveTags(item.id)}>Save</button></div>}</div>
